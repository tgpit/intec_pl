...
class CDeliverySDEK extends sdekHelper{
    ...
    /**
     * Compatibility check method. In this case, it is almost similar to the calculation of the cost.
     * @param $arOrder
     * @param $arConfig
     * @return array|false
     */
    static function Compability($arOrder, $arConfig)
    {
        ...
        if ($arCity) {
            ...
            // Disabling profiles where there are no included tariffs.
            ...
            // Checking the possibility of delivery, if not editing the order.
            if (!(array_key_exists('isdek_action', $_POST) && $_POST['isdek_action'])
                ...
                // Checks pickup points availability against goods dimensions
                $pickupProfilesAvailable = \Ipolh\SDEK\PointsHandler::checkAvailableProfiles(self::$sdekCity, self::$goods);
                foreach ($pickupProfilesAvailable as $profile => $isAvailable) {
                    if (!$isAvailable && array_key_exists($profile, $arKeys)) {
                        unset($arKeys[array_search($profile, $arKeys)]);
                    }
                }
        ...
        // Connecting FrontEnd (for a multi-page component)
	...
	static function getBasketGoods($arFilter=array()){
		...
		$dbBasketItems = CSaleBasket::GetList(
			...
			false,
			false,
			array("ID","PRODUCT_ID", "PRICE", "QUANTITY",'CAN_BUY','DELAY',"NAME","DIMENSIONS","WEIGHT","PRICE","SET_PARENT_ID","LID","CURRENCY","VAT_RATE","MEASURE_NAME","DETAIL_PAGE_URL")
		);
		while ($arItem = $dbBasketItems->Fetch())
			if ($arItem['CAN_BUY'] == 'Y' && $arItem['DELAY'] == 'N'){
                if (!empty($arItem["DIMENSIONS"]) && is_string($arItem["DIMENSIONS"])) {
				...
				if($arItem['QUANTITY'] != intval($arItem['QUANTITY'])){
                    ...
                    $arItem['WEIGHT'] = round($arItem['WEIGHT'] * $arItem['QUANTITY']);
					...
					if(\Ipolh\SDEK\option::get('addMeasureName') == 'Y')
	...
	public static function weightPVZ($weight=false,$src=false){
		...
		if(is_array($arPVZs) && count($arPVZs))
	...
	public static function weightPST($arGabs=false,$src=false){
		...
		if(is_array($arPSTs) && count($arPSTs)) {
	...
	public static function checkPVZ($city = ''){
		...
		return is_array(self::$PVZcities) && array_key_exists($city,self::$PVZcities);
	}
	...
	public static function checkPOSTAMAT($city = ''){
        ...
        return is_array(self::$POSTAMATcities) && array_key_exists($city,self::$POSTAMATcities);
	}
