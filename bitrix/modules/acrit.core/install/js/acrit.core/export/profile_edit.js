...
$(document).ready(function(){
	if(typeof acritExpModuleVersion != 'undefined' && acritExpModuleVersion.length > 0 && acritExpCoreVersion.length > 0) {
...
function acritExpChangeProfileName(){
	...
	if(inputName.attr('data-custom-name') != 'true'){
		...
		acritExpAjax('get_profile_name_index', get, function(JsonResult, textStatus, jqXHR){
...
function acritExpPluginLoadSettings(saveValues) {
	var currentValues = '',
		...
		divFormatNotice = $('#acrit_exp_form div[data-role="format_notice"]');
	if(saveValues){
	...
	acritExpAjax('load_plugin_settings', currentValues, function(JsonResult, textStatus, jqXHR){
		...
		if(typeof JsonResult.FORMAT_NOTICE == 'string' && JsonResult.FORMAT_NOTICE.length){
			divFormatNotice.html(JsonResult.FORMAT_NOTICE);
		}
...
$(document).ready(function(){
	...
	window.acritExpSelect2Matcher = function(params, item){
		function optionIsMatch(option, search){
			return [option.text, option.value, option.value.replace(/_/g, ''), option.title].join(' ').toUpperCase().indexOf(searchText) != -1;
		}
	...
	//
	window.acritExpGroupIdSelect2Config = {
		dropdownParent: $('#tr_GROUP_ID > td > div').first(),
		dropdownPosition: 'below',
		matcher:window.acritExpSelect2Matcher,
		language: 'ru'
	};
	window.acritExpGroupIdSelect2Object = $('#field_GROUP_ID').select2(window.acritExpGroupIdSelect2Config);
...
$(document).delegate('table[data-role="value-replaces"] [data-role="replace-use-regexp"]', 'change', function(e){
	...
	var
		inputModifier = $(this).closest('tr').find('[data-role="replace-regexp-modifier"]');
	if($(this).is(':checked')){
		inputModifier.show();
	}
	else{
...
$(document).delegate('table[data-role="value-replaces"] [data-role="replace-whole"]', 'change', function(e){
	$(this).parent().find('input[type=hidden]').val($(this).prop('checked')?'Y':'N');
});
$(document).delegate('table[data-role="value-replaces"] [data-role="replace-use-regexp"]', 'change', function(e){
...
$(document).ready(function(){
	...
	AcritExpPopupCategoriesRedefinitionSelect.Open = function(openerLink, iblockId, sectionId, currentValue){
		...
		this.Show();
		this.LoadContent();
		$('.bx-core-adm-dialog-buttons', this.DIV).append(`<span>&nbsp; &nbsp; &nbsp; ${BX.message('ACRIT_EXP_POPUP_CATEGORY_REDEFINITION_SELECT_NOTE')}</span>`);
	}
	...
	AcritExpPopupCategoriesRedefinitionSelect.FilterFields = function(){
		...
		acritExpAjax('categories_redefinition_filter', {search: fieldText.val()}, function(JsonResult, textStatus, jqXHR){
			$('select[data-role="category-redefinition-select"]', AcritExpPopupCategoriesRedefinitionSelect.DIV).html(JsonResult.HTML);
			acritExpHandleAjaxError(jqXHR, false);
		}, function(jqXHR){
			acritExpHandleAjaxError(jqXHR, true);
		}, true);
...
$(document).delegate('a[data-role="categories-redefinition-button-picture-clear"]', 'click', function(e){
	e.preventDefault();
	var item = $(this).closest('tr');
	$('[data-role="categories-redefinition-picture"]', item).val('');
});
...
$(document).ready(function(){
	let
		countConfig = parseInt($('span[data-role="acrit_exp_max_input_vars_warning_value_current"]').text()),
		countReal = $('#AcritExpProfile_form :input').length,
		spanCountTarget = $('span[data-role="acrit_exp_max_input_vars_warning_value_target"]'),
		divWarning = $('div[data-role="acrit_exp_max_input_vars_warning"]');
	if(!isNaN(countConfig) && spanCountTarget.length == 1){
		if(countReal > countConfig){
			spanCountTarget.text(countReal);
			divWarning.show();
			$('#AcritExpProfile_form').bind('submit', function(e){
				e.preventDefault();
			});
			$('input[type="button"], input[type="submit"]', '.adm-detail-content-btns').attr('disabled', 'disabled');
			$('#acrit_exp_form [data-role="iblock-settings-save"]').attr('disabled', 'disabled');
			$('#acrit_exp_form [data-role="iblock-settings-clear"]').attr('disabled', 'disabled');
			$('span[data-role="iblock-settings-save-progress"]').attr('data-saving', 'Y'); // prevent save by Alt+S
		}
	}
});
