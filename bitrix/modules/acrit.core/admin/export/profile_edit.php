...
BX.message({
	...
	ACRIT_EXP_POPUP_CATEGORY_REDEFINITION_MASS_EDIT: '<?=Loc::getMessage('ACRIT_EXP_POPUP_CATEGORY_REDEFINITION_MASS_EDIT');?>',
	ACRIT_EXP_POPUP_CATEGORY_REDEFINITION_MASS_EDIT_TITLE: '<?=Loc::getMessage('ACRIT_EXP_POPUP_CATEGORY_REDEFINITION_MASS_EDIT_TITLE');?>',
	...
	ACRIT_EXP_POPUP_CATEGORY_REDEFINITION_SELECT_NOTE: '<?=Loc::getMessage('ACRIT_EXP_POPUP_CATEGORY_REDEFINITION_SELECT_NOTE');?>',
	// Popup: Execute
...
if($intProfileID>0) {
	...
	if(Helper::call($strModuleId, 'Profile', 'getList', [$arQuery])->getSelectedRowsCount() == 0){
		LocalRedirect('/bitrix/admin/acrit_'.$strModuleCodeLower.'_new_list.php?group_id='.intVal($arGet['group_id']).'&lang='.LANGUAGE_ID);
	}
...
if($arGet['delete'] == 'Y'){
	$arDeteleQuery = ['filter' => ['ID' => $intProfileID]];
	if($arDeleteProfile = Helper::call($strModuleId, 'Profile', 'getList', [$arDeteleQuery])->fetch()){
		Helper::call($strModuleId, 'Profile', 'delete', [$intProfileID]);
		$strUrl = '/bitrix/admin/acrit_'.$strModuleCodeLower.'_new_list.php?group_id='.intVal($arGet['group_id']).'&lang='.LANGUAGE_ID;
		if($bGroupsAvailable && $arDeleteProfile['GROUP_ID'] > 0){
			$strUrl .= '&group_id='.$arDeleteProfile['GROUP_ID'];
		}
...
// Download fields
if($arGet['download'] == 'fields' && $intIBlockID){
	(new \Acrit\Core\Export\DownloadFields($strModuleId, $intProfileID, $intIBlockID))->download();
}
...
if(($bSave || $bApply) && $strRight == 'W'){
	...
	if($bCopySuccess || $obResult->isSuccess()) {
		...
		if($bGroupsAvailable && $arProfileFields['GROUP_ID'] > 0){
			$strUrl .= '&group_id='.$arProfileFields['GROUP_ID'];
		}
		LocalRedirect($strUrl);
	}
	else {
...
if(strlen($strAjaxAction)){
	...
	switch($strAjaxAction){
		...
		case 'categories_redefinition_select':
			$arJsonResult['Success'] = true;
			$bCategoriesFilter = false;
			ob_start();
			require __DIR__.'/include/popups/categories_redefinition_select.php';
			$arJsonResult['HTML'] = ob_get_clean();
			break;
		// Categories redefinition [FILTER]
		case 'categories_redefinition_filter':
			$arJsonResult['Success'] = true;
			$bCategoriesFilter = true;
			ob_start();
			require __DIR__.'/include/popups/categories_redefinition_select.php';
			$arJsonResult['HTML'] = ob_get_clean();
			break;
		...
		case 'load_plugin_settings':
			...
			if(is_object($obPlugin)) {
				...
				$arJsonResult['FORMAT_NOTICE'] = $obPlugin::showFormatNotice();
			}
			break;
		// Load popup 'Cron'
			...
			if($bContinue) {
				if(is_object($obPlugin)){
					...
					if($obPlugin->updateCategories($intProfileID, $arAdditionalData)) {
						$arJsonResult['Success'] = true;
						$arJsonResult['Date'] = '---';
						if($strDate = $obPlugin->getCategoriesDate()){
							$arJsonResult['Date'] = Helper::formatUnixDatetime($strDate);
						}
					}
				}
			}
...
$arMenu[] = array(
	...
	'LINK' => 'acrit_'.$strModuleCodeLower.'_new_list.php?group_id='.intVal($arGet['group_id']).'&lang='.LANGUAGE_ID,
	'ICON' => 'btn_list',
);
#
if(!empty($arTeachers)){
...
if($intProfileID) {
	...
	if(!$bCopy){
		$arActionsMenu[] = array(
			...
			'LINK' => 'acrit_'.$strModuleCodeLower.'_new_edit.php?ID='.$intProfileID.'&copy=Y&group_id='.intVal($arGet['group_id']).'&lang='.LANGUAGE_ID,
			'ICON' => 'copy',
		);
...
?><div data-role="acrit_exp_max_input_vars_warning" style="display:none;"><?
print Helper::showError(Helper::getMessage('ACRIT_EXP_MAX_INPUT_VARS_WARNING_TITLE', [
	'#VALUE_CURRENT#' => intVal(ini_get('max_input_vars')),
]), Helper::getMessage('ACRIT_EXP_MAX_INPUT_VARS_WARNING_DETAILS'), true);
?><br/></div><?
...
// Note with help offer
include __DIR__.'/../../include/export/note_profile_help/include.php';

// Top-5 fails
include __DIR__.'/../../include/export/guide_top5_fails/include.php';
