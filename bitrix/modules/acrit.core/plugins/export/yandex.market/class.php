...
class YandexMarket extends Plugin {
	...
	protected $bAllowAlwaysAllCategories = false;
	...
	/**
	 * Output (if need) plugin format notice
	 */
	public static function showFormatNotice(){
		if(Helper::strpos(static::getCode(), 'YANDEX_') === 0){
			return static::getMessage('FORMAT_NOTICE', [
				'#ICON#' => 'data:image/png;base64,'.base64_encode(file_get_contents(__DIR__.'/format_notice.png')),
			]);
		}
	}
	...
	protected function showDefaultSettings(){
		...
		<?endif?>
		<?static::showDefaultSettingsYandexMarketPartner();?>
		<?
		return ob_get_clean();
	}
	 ...
	 *	Show plugin default settings
	 */
	public static function showDefaultSettingsYandexMarketPartner($bOutput=true){
		ob_start();
		if(Helper::strpos(static::getCode(), 'YANDEX_') === 0){
			require __DIR__.'/include/partner.php';
		}
		...
		<table class="acrit-exp-plugin-settings" style="width:100%;">
			<tbody>
				...
				<?endif?>
					<tr>
						<td width="40%" class="adm-detail-content-cell-l">
							<?=Helper::showHint(static::getMessage('EXPORT_NEW_DATE_FORMAT'));?>
							<b><?=static::getMessage('EXPORT_NEW_DATE_FORMAT');?>:</b>
						</td>
						<td width="60%" class="adm-detail-content-cell-r">
	...
	protected function stepExport_writeXmlHeader($intProfileID, $arData){
		...
		if ( $arData['PROFILE']['PARAMS']['EXPORT_NEW_DATE_FORMAT'] != 'Y' )
			$strDate = date('c');
		$strXml = '';
	...
	protected function stepExport_writeXmlCategories($intProfileID, $arData){
		...
		# 
		$bCategoriesByFields = false;
		foreach($arData['PROFILE']['IBLOCKS'] as $intIBlockID => $arIBlockSettings){
			if($arIBlockSettings['PARAMS']['CATEGORIES_REDEFINITION_SOURCE'] == CategoryRedefinition::SOURCE_CUSTOM){
				$bCategoriesByFields = true;
				break;
			}
		}
		if($bCategoriesByFields){
			return $this->stepExport_writeXmlCategories_ByFields($intProfileID, $arData);
		}
		...
		switch($arData['PROFILE']['PARAMS']['CATEGORIES_REDEFINITION_MODE']){
			...
			case CategoryRedefinition::MODE_STRICT:
				...
				$arSectionsForExportAllOriginal = $arSectionsForExportAll;
				if(!empty($arCategoryReduction = $this->categoryReduction())){
					foreach($arSectionsForExportAll as $intSectionID => $arSection){
						if(isset($arCategoryReduction[$intSectionID]) && $arCategoryReduction[$intSectionID] != $intSectionID){
							unset($arSectionsForExportAll[$intSectionID]);
						}
					}
					foreach($arCategoryReduction as $intFrom => $intTo){
						if($intTo && !isset($arSectionsForExportAll[$intTo]) && !isset($arSectionsForExportAll[$intFrom])){
							if(isset($arSectionsForExportAllOriginal[$intFrom])){
								$arSectionsForExportAll[$intTo] = $arSectionsForExportAllOriginal[$intFrom];
							}
						}
					}
				};
				# End of 'Remove duplicate categories'
				...
				foreach($arSectionsForExportAll as $intCategoryID => $arCategory){
					if($arData['PROFILE']['PARAMS']['CATEGORIES_EXPORT_PARENTS']=='Y') {
						...
						while($arSectionsChain = $resSectionsChain->getNext()){
							...
							if($intParentID){
								$arCategoryXml['@']['parentId'] = $intParentID;
							}
						...
						$intParentID = false;
						$strCategoryName = $arCategory['NAME'];
						if(isset($arCategoryRedefinitionsAll[$intCategoryID]) && Helper::strlen($arCategoryRedefinitionsAll[$intCategoryID])){
							$strCategoryName = $arCategoryRedefinitionsAll[$intCategoryID];
						}
						$arCategory = array(
							'@' => array('id' => $intCategoryID),
	 ...
	 * 
	 */
	protected function stepExport_writeXmlCategories_ByFields($intProfileID, $arData){
		...
		foreach($arData['PROFILE']['IBLOCKS'] as $intIBlockID => $arIBlockSettings){
			...
			$resCategories = Helper::call($this->strModuleId, 'CategoryCustomName', 'getList', [$arQuery]);
			while($arCategory = $resCategories->fetch()){
				$arCategoriesAll[$arCategory['CATEGORY_ID']] = $arCategory;
				
			}
		}
		# Convert to XML array
		$arCategoriesXml = [];
		foreach($arCategoriesAll as $intCategoryId => $arCategory){
			$arCategoriesXml[] = [
				'@' => ['id' => $intCategoryId],
				'#' => htmlspecialcharsbx($arCategory['CATEGORY_NAME']),
			];
		}
		# Write XML
		$arXml = [
		...
		$strXml = Xml::arrayToXml($arXml, $intDepthLevel=3);
		$strXml = Helper::convertEncodingTo($strXml, $arData['PROFILE']['PARAMS']['ENCODING']);
		file_put_contents($arData['SESSION']['EXPORT']['XML_FILE_TMP'], $strXml, FILE_APPEND);
	}
		...
		if(!empty($arGiftIDsNotInOffers) && \Bitrix\Main\Loader::includeModule('iblock')){
			...
			while($arItem = $resItems->getNext()){
				...
				if(strlen($strPicture)){
					$strPicture = Helper::siteUrl($arData['PROFILE']['DOMAIN'], $arData['PROFILE']['IS_HTTPS']=='Y', $strPicture);
					$arXml['gift']['#']['picture'] = Xml::addTag($strPicture);
				}
				else{
	...
	*/
	 ...
	 */
	protected function getXmlTag_Condition($strConditionType, $strConditionReason, $strConditionQuality=null){
		$mResult = '';
		if(strlen($strConditionType)){
			if(is_array($strConditionReason)){
				$strConditionReason = implode(', ', $strConditionReason);
			}
			if(is_array($strConditionQuality)){
				$strConditionQuality = implode(', ', $strConditionQuality);
			}
			$mResult = array(
				array(
					'@' => array(
			...
			if(Helper::strlen($strConditionReason)){
				$mResult[0]['#']['reason'] = array(
					array('#' => $strConditionReason),
				);
			}
			if(Helper::strlen($strConditionQuality)){
				$mResult[0]['#']['quality'] = array(
					array('#' => $strConditionQuality),
				);
			}
		}
		return $mResult;
	...
	protected function getXmlTag_Param($arProfile, $intIBlockID, $arFields){
		...
		foreach (EventManager::getInstance()->findEventHandlers($this->strModuleId, 'OnYandexMarketParam') as $arHandler) {
			ExecuteModuleEventEx($arHandler, array(&$mResult, $arProfile, $intIBlockID, $arFields));
		}
		return $mResult;
	}
	...
	protected function processElementPriceLabsParams($intIBlockId, $arFields, &$arXmlTags){
		$arValues = [];
		if(is_array($arPriceLabsParams = $this->getPriceLabsParams($intIBlockId))){
			foreach($arPriceLabsParams as $strHash => $strValue){
	...
	/**
	 * Reduct add sections with same redefinition to one category
	 * @return array of items: [sectionIdReal => sectionIdTarget]
	 */
	protected function categoryReduction(){
		static $arResultCache = [];
		$arResult = &$arResultCache[$this->strModuleId.'_'.$this->intProfileId];
		if(!is_array($arResult)){
			$arResult = [];
			$arRedefinitions = Helper::call($this->strModuleId, 'CategoryRedefinition', 'getForProfile', [$this->intProfileId]);
			$arSectionNames = array_flip(array_unique($arRedefinitions));
			foreach($arRedefinitions as $intSectionId => $strRedefinition){
				$intTargetId = null;
				if(Helper::strlen($strRedefinition) && isset($arSectionNames[$strRedefinition])){
					$intTargetId = intVal($arSectionNames[$strRedefinition]);
				}
				$arResult[$intSectionId] = $intTargetId;
			}
		}
		return $arResult;
	}
	...
	/**
	 * Output tag 'categoryId' to element, considering category unique
	 */
	protected function addTagCategoryId($arElementSections){
		$intSectionId = reset($arElementSections);
		if(!empty($arCategoryReduction = $this->categoryReduction())){
			if(isset($arCategoryReduction[$intSectionId]) && $arCategoryReduction[$intSectionId] != $intSectionId){
				$intSectionId = $arCategoryReduction[$intSectionId];
			}
		}
		return Xml::addTag($intSectionId);
	}
	...
	public function categoriesCustomActions($intIBlockID, $arIBlockParams){
		if(!$this->bAllowAlwaysAllCategories){
			return '';
		}
		return $this->includeHtml(__DIR__.'/include/categories.php', [
			'IBLOCK_ID' => $intIBlockID,
