...
use
	...
	\Acrit\Core\Export\Plugins\OzonRu2024Helpers\Api,
	\Acrit\Core\Export\Plugins\OzonRu2024Helpers\CategoryTable as Category,
	\Acrit\Core\Export\Plugins\OzonRu2024Helpers\AttributeTable as Attribute,
	\Acrit\Core\Export\Plugins\OzonRu2024Helpers\AttributeValueTable as AttributeValue,
	\Acrit\Core\Export\Plugins\OzonRu2024Helpers\TaskTable as Task,
	\Acrit\Core\Export\Plugins\OzonRu2024Helpers\HistoryTable as History,
	\Acrit\Core\Export\Plugins\OzonRu2024Helpers\HistoryStockTable as HistoryStock;
	

class OzonRu2024 extends UniversalPlugin {
	...
	public function getUniversalFields($intProfileId, $intIBlockId){
		...
		if($this->isStockAndPrice()){
			...
			foreach($arData as $arCategory){
				if(!isset($arCategory['TYPE_ID'])){
					continue;
				}
				$arResult['HEADER_CATEGORY_'.$arCategory['TYPE_ID']] = [
					'NAME' => $this->formatTypeName($arCategory['TYPE_ID'], $arCategory['NAME']),
					'NORMAL_CASE' => true,
				];
				foreach($arCategory['ATTRIBUTES'] as $arAttribute){
					$strAttributeId = sprintf(static::ATTRIBUTE_ID, $arCategory['TYPE_ID'], $arAttribute['ATTRIBUTE_ID']);
					$arField = [
	...
	protected function getDataForFields($intIBlockId){
		...
		if(!empty($arUsedCategoriesId)){
			$resCategories = Category::getList([
				'order' => ['NAME' => 'ASC'],
				'filter' => ['TYPE_ID' => $arUsedCategoriesId],
				'select' => ['ID', 'CATEGORY_ID', 'TYPE_ID', 'NAME'],
			]);
			while($arCategory = $resCategories->fetch()){
				$arCategory['ATTRIBUTES'] = [];
				$arResult[$arCategory['TYPE_ID']] = $arCategory;
			}
			$resAttributes = Attribute::getList([
				'order' => ['NAME' => 'ASC'],
				'filter' => ['TYPE_ID' => $arUsedCategoriesId],
				'select' => ['ID', 'CATEGORY_ID', 'TYPE_ID', 'ATTRIBUTE_ID', 'DICTIONARY_ID', 'NAME', 'DESCRIPTION', 'TYPE', 
			...
			while($arAttribute = $resAttributes->fetch()){
				...
				$arResult[$arAttribute['TYPE_ID']]['ATTRIBUTES'][$arAttribute['ATTRIBUTE_ID']] = $arAttribute;
			}
		}
		# Group
	...
	protected function onSetProfileArray(){
		if(!$this->API){
			$this->API = new OzonRu2024Helpers\Api($this->arParams['CLIENT_ID'], $this->arParams['API_KEY'], $this->intProfileId,
				$this->strModuleId);
		}
	}
	...
	public function getCategoriesList($intProfileId){
		...
		$arResult = [];
		$resCategories = Category::getList(['order' => ['NAME' => 'ASC'], 'select' => ['TYPE_ID', 'NAME']]);
		while($arCategory = $resCategories->fetch()){
			$arResult[$arCategory['TYPE_ID']] = $this->formatTypeName($arCategory['TYPE_ID'], $arCategory['NAME']);
		}
	...
	public function updateCategories($intProfileId){
		$bSuccess = false;
		$strCommand = '/v1/description-category/tree';
	...
	protected function processUpdatedCategory($arCategoriesCurrent, $strSessionId, $arName=[], $bRecurred=false){
		if(is_array($arCategoriesCurrent)){
			foreach($arCategoriesCurrent as $arCategory){
				$arNameChain = array_merge($arName, [$arCategory['description_category_id'] => $arCategory['category_name']]);
				if(!empty($arCategory['children']) && isset($arCategory['children'][0]['description_category_id'])){
					$this->processUpdatedCategory($arCategory['children'], $strSessionId, $arNameChain, true);
				}
				else{
					if(is_array($arCategory['children'])){
						foreach($arCategory['children'] as $arChildren){
							$arFields = [
								'CATEGORY_ID' => $arCategory['description_category_id'],
								'TYPE_ID' => $arChildren['type_id'],
								'DISABLED' => $arChildren['disabled'] ? 'Y' : 'N',
								'NAME' => implode($this->strCategoryLevelDelimiter, array_merge($arNameChain, [$arChildren['type_id'] => $arChildren['type_name']])),
	...
	protected function checkAccess($arParams, &$arJsonResult){
		...
		$obApi = new OzonRu2024Helpers\Api($strClientId, $strApiKey, $this->intProfileId, $this->strModuleId);
	...
	protected function ajaxUpdateCategories($arParams, &$arJsonResult){
		...
		if($arPost['start'] == 'Y'){
			...
			$arSession = [
				'ID' => session_id(),
				'CATEGORIES' => $this->getUsedCategories($arPost['iblock_id'], true), // We use here TYPE_ID !!!
				'ATTRIBUTES' => [],
				'COUNT' => 0,
				'INDEX' => 0,
				'CATEGORY_ID' => false,
			...
			foreach($arSession['CATEGORIES'] as $intCategoryId){
				Attribute::deleteByFilter(['TYPE_ID' => $intCategoryId]);
				AttributeValue::deleteByFilter(['TYPE_ID' => $intCategoryId]);
			}
		}
		else{
			...
			# Update attribtutes if it in queue
				...
				foreach($arSession['CATEGORIES'] as $key1 => $intCategoryId){
					...
					$arTypeAttributes = $this->updateCategoryAttrubutes($intCategoryId, $arSession['ID']);
					if(!empty($arTypeAttributes)){
						foreach($arTypeAttributes as $intTypeId => $arAttributes){
					...
					$arSession['CATEGORY_NAME'] = $this->getTypeName($intCategoryId);
			...
			if(is_array($arIBlockParams['CATEGORIES_ALTERNATIVE_LIST'])){
				foreach($arIBlockParams['CATEGORIES_ALTERNATIVE_LIST'] as $intCategoryId){
					if(is_numeric($intCategoryId)){
						$arResult[$intCategoryId] = $this->getTypeName($intCategoryId);
					}
				}
			}
		}
	 ...
	 */
	protected function formatTypeName($intTypeId, $strTypeName=null){
		if(!strlen($strTypeName) && $intTypeId > 0){
			$strTypeName = $this->getTypeName($intTypeId);
		}
		if(!is_numeric($intTypeId) || !$intTypeId){
			return $strTypeName;
		}
		return sprintf('[%d] %s', $intTypeId, $strTypeName);
	}
	...
	protected function updateCategoryAttrubutes($arCategoryId, $strSessionId){
		$strCommand = '/v1/description-category/attribute';
		...
		$arResult = [];
		foreach($arCategoryId as $intTypeId){
			if($arCategory = Category::getList(['filter' => ['TYPE_ID' => $intTypeId], 'select' => ['CATEGORY_ID', 'TYPE_ID']])->fetch()){
				$arJsonRequest = [
					'description_category_id' => intVal($arCategory['CATEGORY_ID']),
					'language' => 'DEFAULT',
					'type_id' => intVal($intTypeId),
				];
				...
				if(is_array($arJsonResponse['result']) && !empty($arJsonResponse['result'])){
					foreach($arJsonResponse['result'] as $arItem){
						...
						if($arFields){
							$arResult[$intTypeId][$arItem['id']] = $arFields;
						}
					}
	...
	protected function updateAttrubuteValues($arAttr, $strSessionId){
		...
		$arJsonRequest = [
			'description_category_id' => $arAttr['CAT'],
			'type_id' => $arAttr['TYPE'],
	 ...
	 */
	protected function isAttributeRequired($intTypeId, $intAttributeId){
		if(!is_array($this->arCacheRequiredAttributes[$intTypeId])){
			$this->arCacheRequiredAttributes[$intTypeId] = [];
			$resQuery = Attribute::getList([
				'filter' => ['TYPE_ID' => $intTypeId, 'IS_REQUIRED' => 'Y'],
				'select' => ['ATTRIBUTE_ID'],
			]);
			while($arItem = $resQuery->fetch()){
				$this->arCacheRequiredAttributes[$intTypeId][$arItem['ATTRIBUTE_ID']] = true;
			}
		}
		return isset($this->arCacheRequiredAttributes[$intTypeId][$intAttributeId]);
	}
	 ...
	 */
	protected function isAttributeDictionary($intTypeId, $intAttributeId){
		if(!is_array($this->arCacheDictionaryAttributes[$intTypeId])){
			$this->arCacheDictionaryAttributes[$intTypeId] = [];
			$arFilter = [
				'filter' => ['>DICTIONARY_ID' => 0],
				'select' => ['ATTRIBUTE_ID', 'DICTIONARY_ID'],
			];
			if(is_numeric($intTypeId)){
				$arFilter['filter']['TYPE_ID'] = $intTypeId;
			}
			$resQuery = Attribute::getList($arFilter);
			while($arItem = $resQuery->fetch()){
				$this->arCacheDictionaryAttributes[$intTypeId][$arItem['ATTRIBUTE_ID']] = $arItem['DICTIONARY_ID'];
			}
		}
		$intDictionaryId = $this->arCacheDictionaryAttributes[$intTypeId][$intAttributeId];
	...
	protected function onUpBuildJson(&$arItem, &$arElement, &$arFields, &$arElementSections, &$arDataMore){
		...
		# Remove 'category_id', add 'description_category_id'
		if(isset($arItem['category_id'])){
			if(is_numeric($arItem['category_id']) && $arItem['category_id'] > 0){
				if($arCategory = Category::getList(['filter' => ['TYPE_ID' => $arItem['category_id']], 'select' => ['CATEGORY_ID']])->fetch()){
					$arItem['description_category_id'] = intVal($arCategory['CATEGORY_ID']);
				}
			}
			unset($arItem['category_id']);
		}
		# Handler
		...
		if(!$this->isStockAndPrice()){
			foreach($arFields as $strField => $mValue){
				if($arAttribute = $this->parseAttributeId($strField)){
					if($arAttribute['TYPE_ID'] != static::GROUPED_CODE && $arAttribute['TYPE_ID'] != $intProductCategoryId){
	...
	protected function checkRequiredFields($intIBlockId, $intElementId, $arFields){
			...
			if($arFieldsAll[$strField]){
				...
				if($bEmpty && $arFieldsAll[$strField]->isCustomRequired()){
					...
					if(is_array($arAttributeId)){
						if($this->isAttributeRequired($arAttributeId['TYPE_ID'], $arAttributeId['ATTRIBUTE_ID'])){
							if(!is_array($arEmptyRequiredFields[$arAttributeId['TYPE_ID']])){
								$arEmptyRequiredFields[$arAttributeId['TYPE_ID']] = [];
							}
							$arEmptyRequiredFields[$arAttributeId['TYPE_ID']][] = $arFieldsAll[$strField]->getName();
						}
					}
				}
			}
		...
		if(!empty($arEmptyRequiredFields)){
			$arErrors = [];
			foreach($arEmptyRequiredFields as $intTypeId => $arErrorFields){
				$resCategory = Category::getList(['filter' => ['TYPE_ID' => $intTypeId], 'select' => ['NAME']]);
				if($arCategory = $resCategory->fetch()){
					$arErrors[] = static::getMessage('ERROR_EMPTY_REQUIRED_FIELDS', [
						'#ELEMENT_ID#' => $intElementId,
						'#CATEGORY#' => $this->formatTypeName($intTypeId, $arCategory['NAME']),
	...
	protected function doMigrateTaskId(){
		...
		while(true){
			$arQuery = [
				...
				'runtime' => [
					'TASK' => [
						'data_type' => '\Acrit\Core\Export\Plugins\OzonRu2024Helpers\TaskTable',
						'reference' => [
