...
class CustomXmlGeneral extends CustomXml {
	...
	public function processElement($arProfile, $intIBlockID, $arElement, $arFields){
		...
		# Support for multiple tags for one element (create tmp wrapper array)
		$strItemStructure = sprintf('<acrit_item_wrapper>%s</acrit_item_wrapper>', $strItemStructure);

		# Prepare XML
		$arDataWrapper = Xml::xmlToArray($strItemStructure);
		if(!is_array($arDataWrapper)){
		...
		foreach($arDataWrapper['acrit_item_wrapper']['#'] as $strRootKey => &$arRootItem){

			# 
			foreach($arRootItem as $intIndex => &$arItem){
				# Prepare data array (make 1st level like as another)
				$arData = [$strRootKey => [$arItem]];
				...
				# Replace data in result array
				$arRootItem[$intIndex] = $arData[$strRootKey][0];
			}
		...
		# Extract just needed data from tmp array
		$arData = $arDataWrapper['acrit_item_wrapper']['#'];
		...
		# Compile result XML
		$arXmlParts = [];
		foreach($arData as $strRootKey => $arItems){
			foreach($arItems as $intIndex => $arItem){
				$arItemXml = [$strRootKey => $arItem];
				$arXmlParts[] = trim(Xml::arrayToXml($arItemXml));
			}
		}
		...
		$arResult = array(
			'TYPE' => 'XML',
			'DATA' => implode(PHP_EOL, $arXmlParts),
			'CURRENCY' => $strCurrency,
	...
	protected function stepExport_getCurrenciesXml($intProfileID, $arData){
		...
		if(empty($arCurrencies) && isset($this->arParams['CURRENCY']['TARGET_CURRENCY'])){
			if(Helper::strlen($strCurrency = $this->arParams['CURRENCY']['TARGET_CURRENCY'])){
				$arCurrencies[$strCurrency] = [
					'CURRENCY' => $strCurrency,
					'RATE' => 1,
				];
	...
	protected function stepExport_getCategoriesXml($intProfileID, $arData){
		...
		if(!empty($arSectionsForExportAll)) {
			...
			while($arSection = $resSections->getNext(false,false)){
				...
				$arSectionsAll[$arSection['ID']] = array(
					...
					'DETAILPICTURE' => $arSection['DETAILPICTURE'],
				);
			}
		...
		# 
		foreach (EventManager::getInstance()->findEventHandlers($this->strModuleId, 'OnCustomXmlCategoriesPrepare') as $arHandler) {
			ExecuteModuleEventEx($arHandler, [$this, &$arSectionsForExportAll, $intProfileID]);
		}
		...
		foreach($arSectionsForExportAll as $intCategoryID => $arCategory){
			...
			if($arData['PROFILE']['PARAMS']['CATEGORIES_EXPORT_PARENTS']=='Y') {
				...
				while($arSectionsChain = $resSectionsChain->getNext()){
					...
					$arCategoryXml = $arXml;
					$strPicture = $arSectionsChain['PICTURE'] ? Helper::siteUrl($this->arProfile['DOMAIN'], $this->arProfile['IS_HTTPS']=='Y', \CFile::getPath($arSectionsChain['PICTURE'])) : '';
					$strDetailPicture = $arSectionsChain['DETAIL_PICTURE'] ? Helper::siteUrl($this->arProfile['DOMAIN'], $this->arProfile['IS_HTTPS']=='Y', \CFile::getPath($arSectionsChain['DETAIL_PICTURE'])) : '';
					$arReplace = array(
						...
						'#PARENT_EXTERNAL_ID#' => '',
						'#PICTURE#' => $strPicture,
						'#DETAIL_PICTURE#' => $strDetailPicture,
					);
			...
			# Without parents
				...
				$strPicture = $arCategory['PICTURE'] ? Helper::siteUrl($this->arProfile['DOMAIN'], $this->arProfile['IS_HTTPS']=='Y', \CFile::getPath($arCategory['PICTURE'])) : '';
				$strDetailPicture = $arCategory['DETAIL_PICTURE'] ? Helper::siteUrl($this->arProfile['DOMAIN'], $this->arProfile['IS_HTTPS']=='Y', \CFile::getPath($arCategory['DETAIL_PICTURE'])) : '';
				$arReplace = array(
					'#ID#' => $intCategoryID,
		...
		# 
		foreach (EventManager::getInstance()->findEventHandlers($this->strModuleId, 'OnCustomXmlCategoriesExport') as $arHandler) {
			ExecuteModuleEventEx($arHandler, [$this, &$arCategoriesXml, $intProfileID]);
		}
		...
		$strStructureGeneral = $arData['PROFILE']['PARAMS']['CUSTOM_XML_STRUCTURE_GENERAL'];
		if(preg_match('#^([\t\s]+)(.*?)\#CATEGORIES\#(.*?)$#m', $strStructureGeneral, $arMatches)){
			$intTabs = preg_match_all('#\t#', $arMatches[1]);
			if(($intSpaces = preg_match_all('#\s#', $arMatches[1])) > 0){
				$intTabs += ceil($intSpaces / 4);
			}
			$intOffset = $intTabs + 1;
		}
		$strXml = Xml::arrayToXml($arCategoriesXml, 1, false);
		$strXml = Xml::addOffset($strXml, $intOffset);
		$strXml = "\n".rtrim($strXml)."\n".str_repeat("\t", max(0, $intOffset-1));
		return $strXml;
	}
