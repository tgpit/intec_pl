...
$MESS[$strLang.'EXPORT_PRICES_BY_1_CHECKBOX'] = 'Выгружать цены по одной (вместо 100)';
	...
	При этом важно понимать, что включение опции значительно замедлит общую выгрузку цен.';


# Settings: export stocks by 1
$MESS[$strLang.'EXPORT_STOCKS_BY_1_CHECKBOX'] = 'Выгружать остатки по одному (вместо 100)';
	$MESS[$strLang.'EXPORT_STOCKS_BY_1_HINT'] = 'Опция позволяет выгружать остатки на каждую карточку отдельно.<br/><br/>
	По умолчанию опция не включена: модуль выгружает по 100 цен за 1 запрос. Но это может создавать ситуации, когда из-за ошибки выгрузки остатка по одному товару остальные также не выгружаются.<br/><br/>
	При этом важно понимать, что включение опции значительно замедлит общую выгрузку остатков.';
