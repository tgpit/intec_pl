...
class CustomExcelGeneral extends CustomExcel {
	...
	public function processElement($arProfile, $intIBlockID, $arElement, $arFields){
		...
		# Ending..
		unset($intProfileID, $intElementID, $arExcelFields, $strCurrency, $strExcel);
		return $arResult;
	}
	...
	public function stepCheck($intProfileID, $arData){
		if(!extension_loaded('xmlwriter')){
			Log::getInstance($this->strModuleId)->add(static::getMessage('NOTICE_NO_XML_WRITER'), $intProfileID);
			print Helper::showError(static::getMessage('NOTICE_NO_XML_WRITER_HEADER'), static::getMessage('NOTICE_NO_XML_WRITER'));
	...
	public function stepExport($intProfileID, $arData){
		...
		Helper::fixPhpSpreadSheetMbstring(true);
		Helper::includePhpSpreadSheet();
		Helper::fixPhpSpreadSheetMbstring(false);
	...
	public function showMessages(){
		if($this->getFormat() == 'XLSX'){
			Helper::showXlsxNoticeZip();
			Helper::showXlsxNoticeXml();
		}
		parent::showMessages();
