...
//	}
    ...
    public function executeHttpClient($strCommand, $posting) {
        $body = json_encode(['posting_number' => [(string) $posting]]);
        $ext_url = static::URL . $strCommand;
        $httpClient = new HttpClient();
        $httpClient->setHeader('Client-Id', $this->strClientId, false);
        $httpClient->setHeader('API-Key', $this->strApiKey, false);
        $httpClient->setHeader('Content-Type', 'application/json', false);
        $httpClient->query('POST', $ext_url , $body);
