...
class Helper {
	...
	public static function convertUtf8($strText) {
		if(!static::isUtf()) {
			return static::convertEncoding($strText, 'UTF-8', 'CP1251');
		}
		return $strText;
	}
	...
	public static function showError($strMessage=null, $strDetails=null, $bCompact=false) {
		if(php_sapi_name() == 'cli'){
			...
			if(static::strlen($strDetails)){
				print $strDetails.PHP_EOL;
			}
			return;
		}
		ob_start();
	...
	/**
	 * Get value from array (by key) and delete this array item
	 */
	public static function arrayExtract(array &$arData, $strKey){
		foreach($arData as $key => $value){
			if($key === $strKey){
				unset($arData[$key]);
				return $value;
	 ...
	 *	Get general tmp dir
	 */
	public static function getTmpDir($bAutoCreate=true, $bRelative=false, $strModuleId=null){
		$strResult = sprintf('%s/%s/.tmp', static::getUploadDir(), $strModuleId ?? ACRIT_CORE);
	...
	public static function roundEx($fValue, $intPrecision=0, $strFunc=false) {
		...
		return call_user_func($strFunc, floatVal($fValue) * $intPow) / $intPow;
	}
	...
	public static function getCatalogArray($intIBlockID) {
		...
		if($intIBlockID > 0) {
			...
			elseif(Loader::includeModule('catalog')) {
				...
				else { // Инфоблок теперь может не быть торговым каталогом, но может иметь торговые предложения
	...
	public static function isPropertyExists($strCode, $intIBlockId){
		...
		if(is_null($bPropertyExists) && Loader::includeModule('iblock')){
	...
	public static function getMeasuresList(){
		$arCachedValue = &static::$arCacheMeasureList;
		if(!is_array($arCachedValue)){
			$arCachedValue = array();
		}
		if(empty($arCachedValue)){
	 ...
	 */
	public static function includePhpSpreadSheet($strVersion=null){
		static $bIncluded;
		if($bIncluded){
			return true;
		}
		...
		#
		if(is_string($strVersion) && strlen($strVersion)){
			$strPhpSpreadsheetVersion = $strVersion;
		}
		elseif(checkVersion(PHP_VERSION, '8.0.0')){
			$strPhpSpreadsheetVersion = '1.29.0';
		}
		elseif(checkVersion(PHP_VERSION, '7.4.0')){
			$strPhpSpreadsheetVersion = '1.25.2';
		}
		elseif(checkVersion(PHP_VERSION, '7.3.0')){
		...
		if(is_dir($strPhpSpreadsheetDir)){
			...
			foreach($arFiles as $strFileName){
				$strPatternClass = '#(^|(abstract|final)\s+)(class|interface|enum)\s+(\w+)(?=\s+((extends\s+\w+)|(implements\s+\w+)))?#mi';
	 ...
	 *	Add notify
	 */
	public static function addNotify($strModuleId, $strMessage, $strTag, $bClose=true){
		$arParams = [
	...
	public static function email($strEmailTo, $strSubject, $strBody, $bHtml=false, $strEmailFrom=false){
		if(!$strEmailFrom){
			$strEmailFrom = Option::get('main', 'email_from');
		}
	 ...
	 * Check user can read
	 */
	public static function canRead($strModuleId){
		static $strRight = null;
		if(!$GLOBALS['USER']->isAuthorized()){
			return false;
		}
		if(is_null($strRight)){
			$strRight = $GLOBALS['APPLICATION']->getGroupRight($strModuleId);
		}
		return $strRight >= 'R';
	}
	 ...
	 */
	public static function canWrite($strModuleId){
	...
	/**
	 * Get URL for admin menu item
	 */
	public static function adminUrl($strFile, $arParams=[], $bAddLang=true){
	...
	/**
	 * Encode/decode emojii (text | array)
	 */
	public static function emojiiAvailable(){
		return class_exists('\Bitrix\Main\Text\Emoji');
	}
	public static function encodeEmojii(&$mText){
		if(!static::emojiiAvailable()){
			return;
		}
		if(is_array($mText)){
			foreach($mText as &$mValue){
				static::encodeEmojii($mValue);
			}
			unset($strValue);
		}
		elseif(is_string($mText) && static::strlen($mText)){
			$mText = \Bitrix\Main\Text\Emoji::encode($mText);
		}
	}
	public static function decodeEmojii(&$mText){
		if(!static::emojiiAvailable()){
			return;
		}
		if(is_array($mText)){
			foreach($mText as &$mValue){
				static::decodeEmojii($mValue);
			}
			unset($strValue);
		}
		elseif(is_string($mText) && static::strlen($mText)){
			$mText = \Bitrix\Main\Text\Emoji::decode($mText);
		}
	}
	
	/**
	 * Get default valie for enum field
	 */
	public static function getEnumDefault($strClass, $strField){
		static $arCache = [];
		$strResult = &$arCache[$strClass][$strField];
		if(!isset($strResult)){
			$strResult = null;
			$strTable = $strClass::getTableName();
			$strSql = sprintf("SHOW FIELDS FROM %s WHERE Field = '%s';", $strTable, $strField);
			if($arField = \Bitrix\Main\Application::getConnection()->query($strSql)->fetch()){
				$strResult = $arField['Default'];
				if(!strlen($strResult)){
					$strResult = reset(static::getEnumValues($strClass, $strField));
				}
			}
		}
		return $strResult;
	 ...
	 */
	public static function getEnumValues($strClass, $strField){
		static $arCache = [];
		$arResult = &$arCache[$strClass][$strField];
		if(!is_array($arResult)){
			$arResult = [];
			$strTable = $strClass::getTableName();
			$strSql = sprintf("SHOW FIELDS FROM %s WHERE Field = '%s';", $strTable, $strField);
			if($arField = \Bitrix\Main\Application::getConnection()->query($strSql)->fetch()){
				if(stripos($arField['Type'], 'enum(') === 0){
					$strEnums = preg_replace('#^enum\((.*?)\)$#i', '$1', $arField['Type']);
					$arEnums = explode(PHP_EOL, str_replace("','", "'".PHP_EOL."'", $strEnums));
					$arResult = array_map(function($strEnum){
						return trim($strEnum, "'");
					}, $arEnums);
					foreach($arResult as $key => $value){
						if($value == ''){
