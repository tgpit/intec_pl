...
class HttpRequest {
	...
	protected static $arHttpRequestHeaders;
	...
	public static function getHttpContent($strURL, $arParams=array()){
		...
		$bGetRequestHeaders = !!$arParams['GET_REQUEST_HEADERS'];
		...
		if($IsCurl) {
			...
			if($bDebug || $bGetRequestHeaders){
				$arCurlInfo = curl_getinfo($Curl);
				static::$arHttpRequestHeaders = preg_split("#\r*\n#", trim($arCurlInfo['request_header']));
				if($bDebug){
					Helper::L(static::$arHttpRequestHeaders);
				}
			}
			...
			if (!empty($arParams['HEADER'])) {
				...
				if($bGetRequestHeaders){
					static::$arHttpRequestHeaders = preg_split("#\r*\n#", trim($arParams['HEADER']));
				}
			}
			if (trim($arParams['CONTENT'])!='') {
	...
	public static function getRequestHeaders(){
		return static::$arHttpRequestHeaders;
	}
