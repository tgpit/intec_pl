...
class Xml {
	...
	/**
	 *	Remove all offsets in XML string
	 */
	public static function removeOffset($strXML){
		$strXML = preg_replace('#^[\s]+#m', '', $strXML);
		return $strXML;
	}
	...
	/**
	 *	Helper for add tag
	 *	Simple mode <tag attr_name="attr_value"></tag>
	 */
	public static function addEmptyTagWithAttr($strAttrName, $mAttrValue){
		$arResult = array();
		if(is_array($mAttrValue)){
			foreach($mAttrValue as $strValueItem){
				$arResult[] = array(
					'@' => [$strAttrName => $strValueItem],
					'#' => '',
				);
			}
		}
		else {
			$arResult['@'] = [$strAttrName => $mAttrValue];
			$arResult['#'] = '';
		}
		return $arResult;
	}
