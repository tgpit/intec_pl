...
abstract class UniversalPlugin extends Plugin{
	...
	protected $bXmlAddItemOffset = true;
	# JSON settings
	...
	# Extra
	protected $strFilenameSuffix = null;
	...
	public static function getDescription() {
		$strHtml = '';
		static $obParsedown = null;
		if(is_null($obParsedown)){
			if(is_file($strParsedown = Helper::root().'/bitrix/modules/acrit.core/include/parsedown/Parsedown.php')){
				require_once($strParsedown);
				if(class_exists('\Acrit\Core\Parsedown')){
					$obParsedown = new \Acrit\Core\Parsedown;
				}
			}
		}
		#
		if(static::isSubClass()){
			...
			#
			if($obParsedown){
				$strHtml = $obThis->includeFile('.description.md', function($strFile)use($obParsedown){
					return $obParsedown->text(file_get_contents($strFile));
				});
			}
			if(!strlen($strHtml)){
		...
		if($this->areDownloadedCategoriesCorrect($strFileContent)){
			...
			if(file_put_contents($strTmpFile, $strFileContent)){
				...
				if($strCategoriesResult === false){
					$arFileContents = file($strTmpFile, FILE_IGNORE_NEW_LINES  | FILE_SKIP_EMPTY_LINES);
					if(is_array($arFileContents)){
						...
						Log::getInstance($this->strModuleId)->add('Error getting file contents: '.$strTmpFile.' (size: '.filesize($strTmpFile).')');
					}
				}
				else{
	...
	protected function showSettingsEncoding(){
		...
		$strHidden = (count($arEncodings) <= 1 ? static::SETTINGS_HIDDEN : '');
		$arEncodings = array(
	...
	/**
	 * Get main iblock id ($intIBlockId is main iblock id or offers iblock id)
	 */
	protected function getMainIBlockId($intIBlockId){
		$arCatalog = Helper::getCatalogArray($intIBlockId);
		return $arCatalog['PRODUCT_IBLOCK_ID'] ? intVal($arCatalog['PRODUCT_IBLOCK_ID']) : intVal($arCatalog['IBLOCK_ID']);
	}

	/**
	 * Get current offer mode (for selected iblock)
	 **/
	protected function getOffersMode($intIBlockId){
		$intIBlockId = $this->getMainIBlockId($intIBlockId);
		return $this->arProfile['IBLOCKS'][$intIBlockId]['PARAMS']['OFFERS_MODE'];
	}
	protected function isOffersModeOnly($intIBlockId){
		return $this->getOffersMode($intIBlockId) == 'only';
	}
	protected function isOffersModeAll($intIBlockId){
		return $this->getOffersMode($intIBlockId) == 'all';
	}
	protected function isOffersModeNone($intIBlockId){
		return $this->getOffersMode($intIBlockId) == 'none';
	}
	protected function isOffersModeOffers($intIBlockId){
		return $this->getOffersMode($intIBlockId) == 'offers';
	}
	...
	public function stepCheck($intProfileID, $arData){
		...
		if($this->bApi){
			...
			$arExportSteps['EXPORT_PREPARE'] = array(
				'NAME' => 'Prepare exporting',
				'SORT' => 100,
				'FUNC' => 'stepExport_ExportPrepare',
			);
	...
	/**
	 *	Export step: Prepare export
	 */
	protected function stepExport_ExportPrepare($arStep){
		return call_user_func_array(array($this, $this->getMethod(__FUNCTION__)), array($arStep));
	}
	...
	protected function stepExport_ExportItems($arStep){
		...
		while($arItems = $this->getExportDataItems()){
			foreach($arItems as $arItem){
				...
				if($bStepByStep){
					$intCmp = 0;
					if(is_numeric($arItem['SORT']) && is_numeric($this->arStepByStep['LAST_ITEM']['SORT'])){
						$intCmp = $arItem['SORT'] - $this->arStepByStep['LAST_ITEM']['SORT'];
					}
					else{
						$intCmp = strcmp(toLower($arItem['SORT']), toLower($this->arStepByStep['LAST_ITEM']['SORT']));
					}
					if($this->arProfile['PARAMS']['SORT_ORDER'] == 'DESC'){
						$intCmp *= -1;
					}
	...
	protected function processElement_BuildXml($arElement, $arFields, $arElementSections, $intMainIBlockId){
		...
		if(empty($arResult)){
			...
			$arTmpResult = $this->handler('onUpBuildXml', array(&$arXmlTags, &$arXmlAttr, &$strXmlItem, &$arElement,
				&$arFields, &$arElementSections, &$mDataMore));
	...
	protected function processElement_BuildXml_CombineTagsWithAttributes(&$arXmlTags, $arFieldsTags, $arFieldsAttr){
		...
		if(!empty($arCombineTags)){
			foreach($arCombineTags as $strTag => $arTagData){
				if(is_array($arTagData['VALUE']) && count($arTagData['VALUE']) > 1){
	...
	protected function stepExport_XML_ExportCurrencies(){
		...
		if(preg_match('/^(\s*)#EXPORT_CURRENCIES#$/mi', $this->arData['SESSION']['EXPORT']['XML'], $arMatch)){
			...
			if(empty($arCurrencies) && strlen($strBaseCurrency)){
				$arCurrencies[$strBaseCurrency] = array(
			...
			#
			$this->handler('onUpGetXmlCurrencies', array(&$arCurrencies));
			#
			foreach($arCurrencies as $key => $arCurrency){
				$arRub = ['RUB', 'RUR'];
				$bBothRub = in_array($arCurrency['CURRENCY'], $arRub) && in_array($strBaseCurrency, $arRub);
				if($arCurrency['CURRENCY'] == $strBaseCurrency || $bBothRub){
					$strRate = '1';
				}
				else {
	...
	protected function stepExport_XML_ExportItem($arItem){
		...
		if(strlen($arItem['DATA'])){
			$strXml = $arItem['DATA'];
			if($this->bXmlAddItemOffset){
				$strXml = rtrim(Xml::addOffset($strXml, $this->intXmlDepthItems)).static::EOL;
			}
	...
	protected function getJsonEncodeOptions(){
		$intResult = JSON_PRETTY_PRINT;
		if(defined('JSON_INVALID_UTF8_SUBSTITUTE')){
			$intResult = $intResult | JSON_INVALID_UTF8_SUBSTITUTE;
		}
		return $intResult;
	}
	...
	protected function stepExport_JSON_ExportPrepare(){
		return Exporter::RESULT_SUCCESS;
	}
	...
	/**
	 *	Get count of export data items
	 */
	protected function getExportDataItemsCount($arFilter=null, $bWithExported=false){
		$intResult = 0;
		#
		$arBaseFilter = [
			'PROFILE_ID' => $this->arProfile['ID'],
	...
	protected function stepExport_ReplaceFile(&$arSession, $arStep){
		...
		if(!@rename($strFrom, $strTo)){
			$strDetails = static::getMessage('ERROR_REPLACE_TMP_FILE_DETAILS', array(
				...
				'#FILENAME_REAL#' => $arSession['EXPORT_FILE_NAME'],
			));
			if(!$this->bCron) {
	...
	protected function writeToFile($strContent, $strFilenameSuffix=null, $bAutoConvert=true){
		if(is_null($strFilenameSuffix) && !is_null($this->strFilenameSuffix)){
			$strFilenameSuffix = $this->strFilenameSuffix;
		}
	...
	/**
	 * Set custom filename suffix
	 */
	protected function setFilenameSuffix($strFilenameSuffix=null, $bDeleteFile=true){
		$this->strFilenameSuffix = $strFilenameSuffix;
		if($bDeleteFile){
			if(is_file($strFile = $_SERVER['DOCUMENT_ROOT'].$this->getExportFilenameTmp($strFilenameSuffix))){
				@unlink($strFile);
			}
		}
	}
	 ...
	 */
	protected function getFilenameSuffix($strFilenameSuffix=null){
		return $_SERVER['DOCUMENT_ROOT'].$this->getExportFilenameTmp($strFilenameSuffix);
	}
	...
	/**
	 * Try to decode json, return array (if success) or false (if error)
	 */
	protected function tryJsonDecode($strJson){
		try{
	 ...
	 * $strMethod is 'CategoryRedefinition::getForProfile'
	 */
