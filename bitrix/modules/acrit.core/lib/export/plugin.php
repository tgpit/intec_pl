...
/* This class is deprecated! Use UniversalPlugin instead */
...
abstract class Plugin {
	 ...
	 */
	public function getCategoriesListFiltered($intProfileID, $strSearch='', $intLimit=0):array{
		$arResult = [];
		//
		if(is_array($arCategories = $this->getCategoriesList($intProfileID))){
			$strSearch = toLower(trim($strSearch));
			if($strSearch == ''){
				if($intLimit > 0){
					$arResult = array_slice($arCategories, 0, $intLimit);
				}
				else{
					$arResult = $arCategories;
				}
			}
			else{
				$arSearch = Helper::splitSpaceValues($strSearch);
				$intSearchParts = count($arSearch);
				foreach($arCategories as $intCategoryId => $strCategory){
					$strSearchHaystack = toLower($strCategory);
					$intMatch = 0;
					foreach($arSearch as $strSearchItem){
						if(strpos($strSearchHaystack, $strSearchItem) !== false){
							$intMatch++;
						}
					}
					if($intMatch == $intSearchParts){
						$arResult[$intCategoryId] = $strCategory;
					}
					if($intLimit > 0 && count($arResult) >= $intLimit){
						break;
					}
				}
			}
	...
	/**
	 *	Is this request by ajax for load plugin settings?
	 */
	public function setAjaxLoadSettings($bAjaxLoadSettings){
		$this->bAjaxLoadSettings = $bAjaxLoadSettings;
	}
							...
							<tr id="row_STEP_BY_STEP_INDEX">
								...
								<td width="60%" class="adm-detail-content-cell-r">
									<span style="vertical-align:middle;" data-role="step-export-index">
										<?if($arLastExportedItem['STEP'] > 0):?>
											<b><?=$arLastExportedItem['STEP'];?></b>
										<?else:?>
											0
										<?endif?>
									</span>
	...
	protected function getExportFilenameTmp($strSuffix=null, $bTmpExtension=true){
		...
		$strSuffix = is_string($strSuffix) || is_int($strSuffix) ? '.'.$strSuffix : '';
	...
	/**
	 *	Get options for selected IBlock
	 */
	public function &getIBlockParams($intIBlockId){
		if($arCatalog = Helper::getCatalogArray($intIBlockId)){
			# If used offers iblock id, replace it to parent iblock_id
			if($arCatalog['PRODUCT_IBLOCK_ID']){
				$intIBlockId = $arCatalog['PRODUCT_IBLOCK_ID'];
			}
		}
		return $this->arProfile['IBLOCKS'][$intIBlockId]['PARAMS'];
	}
	...
	*/
	...
	/**
	 *	Handler 'onRedefinitionCategoryAfterSave'
	 */
	...
	/**
	 *	Get last exported item
	 */
	public function getLastExportedItem($intProfileID){
		$arResult = [];
		$arFilter = ['ID' => $intProfileID];
		$arSelect = ['LAST_EXPORTED_ITEM'];
		$arQuery = ['filter' => $arFilter, 'select' => $arSelect];
		if($arProfile = Helper::call($this->strModuleId, 'Profile', 'getList', [$arQuery])->fetch()){
			if(is_array($arLastExportedItem = unserialize($arProfile['LAST_EXPORTED_ITEM']))){
				$arResult = $arLastExportedItem;
			}
		}
		return $arResult;
	}
	...
	public function addToLog($strMessage, $bDebug=false){
		if($this->arProfile['PARAMS']['LOG_DEBUG_MODE'] == 'Y'){
			$bDebug = false; // If debug mode for current profile, all debug-messages will be regular (non-debug)
		}
	...
	protected function processExportData($mCallback, &$arParams=[]){
		...
		while(true){
			...
			while($arItem = $resItems->fetch()){
				...
				# Process callback
				$bResult = call_user_func_array($mCallback, [$arItem, &$arParams]);
				if($bResult !== false){
	...
	/**/
	public function addHtmlOptionsGeneralTop($intIBlockID){
		return '';
	}
	...
	public function isPreview(){
		return defined('ACRIT_CORE_EXPORT_PREVIEW') && ACRIT_CORE_EXPORT_PREVIEW === true;
	}
