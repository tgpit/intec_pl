...
abstract class Backup {
	...
	*/
	...
	public static function fileToZip($strTmpFile){
		...
		$GLOBALS['USER'] = is_a($GLOBALS['USER'], '\CUser') ? $GLOBALS['USER'] : new \CUser; // Fix bug with agents on askaron.agents
	...
	public static function restoreFromBackupFile($strFileTmp, $strMode=self::MODE_DEFAULT){
		...
		if(strlen($strContents)){
			...
			# If backup in CP1251, but site already in UTF-8
			if($arProfiles === false && Helper::isUtf()){
				$arProfiles = Helper::convertEncoding(unserialize(Helper::convertEncoding($strContents, 'UTF-8', 'CP1251')), 'CP1251', 'UTF-8');
				if(is_array($arProfiles)) {
					foreach($arProfiles as &$arProfile){
						$arProfile['PARAMS'] = serialize(Helper::convertEncoding(unserialize(Helper::convertEncoding($arProfile['PARAMS'], 'UTF-8', 'CP1251')), 'CP1251', 'UTF-8'));
						if(is_array($arProfile['IBLOCKS'])){
							foreach($arProfile['IBLOCKS'] as &$arIBlock){
								$arIBlock['PARAMS'] = serialize(Helper::convertEncoding(unserialize(Helper::convertEncoding($arIBlock['PARAMS'], 'UTF-8', 'CP1251')), 'CP1251', 'UTF-8'));
							}
							unset($arIBlock);
						}
					}
