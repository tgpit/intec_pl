...
class Field {
	...
	/**
	 *	Set main element id (for main iblock, or if it offers - for parent iblock)
	 */
		...
		if(!is_array($arResult) || empty($arResult)) {
			...
			foreach(get_declared_classes() as $strClass){
				if(is_subclass_of($strClass, __NAMESPACE__.'\ValueBase')) {
					...
					if(!in_array($this->strModuleId, array_slice(\Acrit\Core\Export\Exporter::getExportModules(true), -2))){
	...
	/**
	 *	Getter for $bAllowedValuesCustom
	 */
	public function isAllowedValuesCustom(){
		return $this->bAllowedValuesCustom;
	}
	...
	public function displayRow(){
		...
		<?if($this->isHeader()):?>
			<tr class="heading acrit-exp-fields-table-heading<?if($this->bNormalCase):?> acrit-exp-fields-table-heading-normal-case<?endif?>" data-header="<?=$strFieldCode;?>">
				<td colspan="4">
					<?=$strName;?><?=$strNameSuffix;?>
					<?if(strlen($strHint)):?>
						<?=Helper::showHint($strHint);?>
					<?endif?>
				</td>
			</tr>
		<?else:?>
