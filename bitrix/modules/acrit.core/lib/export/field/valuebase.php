...
abstract class ValueBase {
	 ...
	 */
	public static function getMessage($strKey, $arReplace=null){
		$strCode = static::getCode();
		$strKey = 'ACRIT_EXP_FIELDVALUE_'.$strCode.'_'.$strKey;
		return Loc::getMessage($strKey, $arReplace); 
	}
	...
	public function processSingleValue($arValue, $arElement, $arProfile, $obField){
		...
		if($arValue['TYPE']=='FIELD') {
			...
			if(array_key_exists($strFieldValue, $arAvailableFields['fields']['ITEMS'])){
				if($strFieldValue=='__IBLOCK_SECTION_ID_MORE') {
					$mResult = [];
					$resMoreGroups = \CIBlockElement::getElementGroups($arFields['ID'], true, ['ID']);
					while($arMoreGroup = $resMoreGroups->fetch()){
						if($arMoreGroup['ID'] != $arFields['IBLOCK_SECTION_ID']){
							$mResult[] = intVal($arMoreGroup['ID']);
						}
					}
				}
				elseif($strFieldValue=='__IBLOCK_SECTION_CHAIN') {
				...
				elseif($strFieldValue == 'DETAIL_PAGE_URL'){
					$mResult = Helper::siteUrl($arProfile['DOMAIN'], $arProfile['IS_HTTPS']=='Y', $arFields[$strFieldValue]);
				}
				...
				elseif($strFieldValue == 'CREATED_BY__NAME'){
					$mResult = $this->getUserName($arFields['CREATED_BY']);
				}
				// Modified by name
				elseif($strFieldValue == 'MODIFIED_BY__NAME'){
					$mResult = $this->getUserName($arFields['MODIFIED_BY']);
				}
				// SEO
													...
													if($arItem = $resItem->fetch()) {
														...
														if($arField = $resField->getNext(false,false)){
															if($arField['USER_TYPE_ID']=='file'){
																if(is_numeric($arItem[$strSubField]) && $arItem[$strSubField]>0){
																	$arFile = \CFile::getFileArray($arItem[$strSubField]);
																	$arItem[$strSubField] = Helper::siteUrl($arParams['PROFILE']['DOMAIN'], $arParams['PROFILE']['IS_HTTPS']=='Y', $arFile['SRC']);
																}
																elseif(is_array($arItem[$strSubField])){
																	foreach($arItem[$strSubField] as $key => $intFileId){
																		$arFile = \CFile::getFileArray($intFileId);
																		$arItem[$strSubField][$key] = Helper::siteUrl($arParams['PROFILE']['DOMAIN'], $arParams['PROFILE']['IS_HTTPS']=='Y', $arFile['SRC']);
																	}
																}
															}
														}
									...
									if($arElement = $resElements->getNext()){
										if(in_array($strSubField, array('PREVIEW_PICTURE', 'DETAIL_PICTURE')) && is_numeric($arElement[$strSubField])){
											...
											if(strlen($arElement[$strSubField])){
												$arElement[$strSubField] = Helper::siteUrl($arParams['PROFILE']['DOMAIN'], $arParams['PROFILE']['IS_HTTPS']=='Y', $arElement[$strSubField]);
											}
										}
									...
									if(is_array($arFile)){
										if(true || $arParams['VALUE']['_IS_CONST']){
											...
											if(strlen($arFile['SRC'])){
												return Helper::siteUrl($arParams['PROFILE']['DOMAIN'], $arParams['PROFILE']['IS_HTTPS']=='Y', $arFile['SRC']);
											}
											else {
												return '';
											}
			...
			elseif($strFieldValue == 'CATALOG_VAT_VALUE_YANDEX'){
				$strVatValue = Helper::getVatValueByID($arFields['CATALOG_VAT_ID']);
			...
			# Catalog vat value (7, 2, 5, 6)
			elseif($strFieldValue == 'CATALOG_VAT_VALUE_YANDEX_API'){
				$strVatValue = Helper::getVatValueByID($arFields['CATALOG_VAT_ID']);
				if(is_numeric($strVatValue)){
					$arMap = [
						7 => 20,
					...
					if($vat = array_search(intVal($strVatValue), $arMap)){
						$strVatValue = $vat;
					}
				}
				else{
					$strVatValue = '6'; // No VAT
				}
				$mResult = $strVatValue;
			}
			...
			elseif(preg_match('#^CATALOG_(.*?)$#', $strFieldValue, $arMatch)){
				switch($arMatch[1]){
					...
					case 'MEASURE_RATIO':
						$mResult = 1;
						$arQuery = ['filter' => ['PRODUCT_ID' => $arElement['ID']], 'select' => ['RATIO']];
						if($arMeasure = \Bitrix\Catalog\MeasureRatioTable::getList($arQuery)->fetch()){
							$mResult = $arMeasure['RATIO'];
						}
						break;
					default:
						$mResult = $arFields[$strFieldValue];
				...
				if(is_array($arFields['SECTION']) && !empty($arFields['SECTION'])) {
					...
					if(preg_match('#^SEO_(.*?)$#', $strField, $arMatch)){
						...
						if(is_string($mResult) && strlen($mResult) && in_array($strField, array('PICTURE', 'DETAIL_PICTURE'))){
							$mResult = Helper::siteUrl($arProfile['DOMAIN'], $arProfile['IS_HTTPS']=='Y', $mResult);
						}
					}
				}
			}
	...
	public function constReplaceCallback($strMatch, $arValue, $arElement, $arProfile, $obField){
		...
		$arValue['PARAMS'] = []; // Ticket 5300: we shouldn't apply const params to this field
	...
	protected function getUserName($intUserId){
		static $arUsersCache = [];
		if(!isset($arUsersCache[$intUserId])){
			$arFilter = ['ID' => $intUserId];
			if($arUser = \CUser::getList($by='ID', $order='ASC', $arFilter, ['FIELDS' => ['NAME', 'LAST_NAME']])->fetch()){
				$arUsersCache[$intUserId] = trim($arUser['NAME'].' '.$arUser['LAST_NAME']);
			}
		}
		return $arUsersCache[$intUserId];
	}
