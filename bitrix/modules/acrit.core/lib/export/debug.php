...
class Debug {
	...
	/**
	 *	Find by filter array
	 */
	public static function findByFilter($arFilter, $bUseProfileFilter=true){
		static::_getProduct([], null, $arFilter);
	}
	...
	/**
	 *	Get filter (php-code) for main IBlock
	 */
	public static function getFilter(){
		static::printData(var_export(Helper::call(static::$strModuleId, 'Profile', 'getFilter', [static::$arData['PROFILE_ID'], static::$arData['IBLOCK_ID']]), true));
	}
	
	/**
	 *	Get filter (php-code) for offers IBlock
	 */
	public static function getOffersFilter(){
		static::printData(var_export(Helper::call(static::$strModuleId, 'Profile', 'getFilter', [static::$arData['PROFILE_ID'], static::$arData['OFFERS_IBLOCK_ID']]), true));
	}
	...
	protected static function _showFilterSql($intIBlockID){
		...
		foreach($DB->arQueryDebug as $obSqlTracker){
			...
			if(!preg_match('#^\s*SELECT\s+BE.ID\b#', $strSql)){
				continue;
			}
