...
abstract class ProfileIBlockTable extends Entity\DataManager {
	...
	public static function getAvailableElementFields($intIBlockID){
		...
		$arResult = array(
			...
			'__IBLOCK_SECTION_ID_MORE' => array(
				'NAME' => Loc::getMessage('ACRIT_EXP_ELEMENT_FIELD__IBLOCK_SECTION_ID_MORE'),
				'TYPE' => 'N',
				'FILTRABLE' => false,
			),
		...
		if($intIBlockID && \Bitrix\Main\Loader::includeModule('iblock') && \Bitrix\Main\Loader::includeModule('catalog')){
			...
			if(is_array($arCatalog) && $arCatalog['IBLOCK_ID']==$intIBlockID){
				$arResult = array(
					...
					'VAT_VALUE_YANDEX_API' => array(
						'NAME' => Loc::getMessage('ACRIT_EXP_ELEMENT_FIELD__CATALOG_VAT_VALUE_YANDEX_API'),
						'TYPE' => 'S',
						'READONLY' => true,
						'FILTRABLE' => false,
					),
					...
					'MEASURE_RATIO' => array(
						'NAME' => Loc::getMessage('ACRIT_EXP_ELEMENT_FIELD__CATALOG_MEASURE_RATIO'),
						'TYPE' => 'N',
						'FILTRABLE' => false,
					),
				);
	...
	public static function checkIBlock($intIBlockID){
		...
		$strResult .= '</ul>';
		return $bErrors && strlen($strResult) ? Helper::showError(null, $strResult).'<br/>' : '';
	}
