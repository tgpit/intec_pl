BX.Vue.component ('CONFIRM_WB', {
    ...
    data(){
        return {
            ...
            supplies: [],
            supply: '',
            popup_supp_vis: false,
            popup_create_supp_vis: false,
            supply_name: 'Без названия',
        }
    },
    methods: {
        set_date(event) {
        ...
        get_orders() {
            ...
            this.$parent.$parent.executeBase( orders_req  ).success( data => {
                ...
                this.choice_supply(this.supply);
            });
        },
        get_supplies() {
            this.spinner = true;
            ...
            this.$parent.$parent.executeBase( orders_req  ).success( data => {
                ...
                if ( this.supplies.length > 0 ) {
                    this.popup_supp_vis = true;
                } else {
                    this.popup_create_supp_vis = true;
                }
                this.spinner = false;
        ...
        choice_supply(supp_id) {
            this.supply = supp_id;
            for (let i = 0; i < this.orders.length; i++) {
                this.orders[i].SUPPLY = supp_id;
            }
            // console.log(this.orders);
            this.popup_supp_vis = false;
        },

        new_supply() {
            this.popup_create_supp_vis = false;
            let orders_req = {
                'action': 'new_supply',
    ...
    template: `
        <div class="acrit_orders_wrapper">
            <div class="popup_supplies_wrapper" v-if="popup_supp_vis">
                 <div class="popup_supplies">
                    <div class="popup_supplies_hr acrit_popup_close" @click="popup_supp_vis=false">+</div>
                    <div class="popup_supplies_hr">                      
                       <div class="popup_supplies_td">{{this.mess.choiceSupply}}</div>                       
                    </div> 
                    <div class="popup_supplies_tr" v-for="(supp, key) in this.supplies" :key="key" @click="choice_supply(supp.id)">
                        <div class="popup_supplies_td">{{ supp.id }}</div>
                        <div class="popup_supplies_td">{{ supp.name }}</div>
                        <div class="popup_supplies_td">{{ supp.date }}</div>
                    </div>
                 </div>
            </div> 
            <div class="popup_supplies_wrapper" v-if="popup_create_supp_vis">
                <div class="popup_supplies">
                     <div class="popup_supplies_hr acrit_popup_close" @click="popup_create_supp_vis=false">+</div>
                     <div class="popup_supplies_hr">
                        <div class="popup_supplies_td">{{this.mess.enterSupply}}</div>
                     </div>   
                     <div class="popup_supplies_hr">
                        <div class="popup_supplies_td"><input type="text" name="supply_name" v-model="supply_name"></div>
                        <div class="popup_supplies_td"><button type="button" class="adm-btn"  @click="new_supply()">{{ this.mess.enterOk }}</button></div>
                     </div>    
                </div>
            </div>               
            <div class="acrit_orders_header">            
                ...
                <div><button type="button" class="adm-btn"  @click="popup_create_supp_vis = true;">{{this.mess.createSupply}}</button></div>
                <div><button type="button" class="adm-btn"  @click="get_supplies()">{{ this.mess.choiceSupply }}</button></div>      
                <div class="supplay_id" v-if="this.supply">{{ this.supply }}</div>            
            </div>
